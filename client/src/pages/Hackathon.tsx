/*
 * AOCN Hackathon Hub — Ice Blue + Violet (Coming Soon)
 * 技术栈预览 + 执行路线图
 */
...
export default function Hackathon() {
  ...
  const phases = [
    ...
    { num: '03', title: t('Demo 展示', 'Demo'), desc: t('展示真实查询 (e.g., Renacrypt Pack 分析)，量化 $4M 基数提升', 'Showcase real queries (e.g., Renacrypt Pack analysis), quantify $4M base improvement') },
  ...
  return (
    <div>
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-br from-ice/[0.03] to-violet/[0.03]" />
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet/10 border border-violet/20 mb-6">
            <Clock className="w-3.5 h-3.5 text-violet" />
            <span className="text-[12px] font-medium text-violet">COMING SOON</span>
          </div>
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-[13px] text-white/45">{t('预计时间: 2026年4月', 'Expected: April 2026')}</span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="mb-8">
        <h2 className="text-sm font-semibold text-white/55 mb-3 flex items-center gap-2">
          <Code2 className="w-4 h-4 text-ice-dim" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {techStack.map((tech, i) => (
            ...
            >
              <div className="w-9 h-9 rounded-lg bg-ice/10 flex items-center justify-center shrink-0">
                <tech.icon className="w-4 h-4 text-ice-dim" />
              </div>
              <div>
      ...
      <div>
        <h2 className="text-sm font-semibold text-white/55 mb-3 flex items-center gap-2">
          <Rocket className="w-4 h-4 text-violet-dim" />
        ...
        <div className="relative">
          ...
          <div className="space-y-3">
            {phases.map((phase, i) => (
              ...
              >
                <div className="absolute left-2.5 top-4 w-4 h-4 rounded-full bg-violet/10 border border-violet/20 flex items-center justify-center">
                  <div className="w-1.5 h-1.5 rounded-full bg-violet/40" />
                </div>
                <div className="glass-card rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-mono font-bold text-ice/50">PHASE {phase.num}</span>
                  </div>
                  <h3 className="text-sm font-medium text-white/70 mb-1">{phase.title}</h3>
                  <p className="text-[11px] text-white/30">{phase.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
